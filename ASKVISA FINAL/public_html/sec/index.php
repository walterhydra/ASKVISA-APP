...
header("Content-Security-Policy: default-src 'self'; script-src 'self' 'unsafe-inline' https://cdnjs.cloudflare.com/ajax/libs/ https://cdn.jsdelivr.net/; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com/ https://cdnjs.cloudflare.com/ajax/libs/; font-src 'self' https://fonts.gstatic.com/ https://cdnjs.cloudflare.com/ajax/libs/; img-src 'self' data: https:;");
...
function validateFileName($filename) {
    ...
    if (strpos($filename, chr(0)) !== false) return false;
...
// Handle create order request with CSRF protection
if (isset($_POST['create_order'])) {
    ...
    try {
        // Get data from session with validation
        $country_id = validateInteger($_SESSION['country_id'] ?? 0, 1);
        $country_name = sanitizeInput($_SESSION['country_name'] ?? '');
        $total_people = validateInteger($_SESSION['total_people'] ?? 1, 1, 20);
        $order_contact_email = sanitizeInput($_SESSION['order_contact_email'] ?? '');
        $order_contact_phone = sanitizeInput($_SESSION['order_contact_phone'] ?? '');
        
        if (!$country_id || !validateEmail($order_contact_email)) {
            echo json_encode(['success' => false, 'message' => 'Missing or invalid required information']);
            exit;
        }
        ...
        $temp_order_id = 'TMP_' . time() . '_' . bin2hex(random_bytes(4));
        ...
        echo json_encode([
            ...
            'payment_amount' => $payment_amount,
            'currency' => $currency,
            'redirect_url' => 'payment_page.php?temp_order_id=' . urlencode($temp_order_id),
            'csrf_token' => $_SESSION['csrf_token']
        ]);
        
    } catch (Exception $e) {
        error_log('Error preparing payment: ' . $e->getMessage());
        echo json_encode(['success' => false, 'message' => 'Error preparing payment']);
    }
    exit;
}
...
if (isset($_GET['check_payment_status'])) {
    ...
    if (isset($_SESSION['payment_success_order_id'])) {
        $response = [
            'success' => true, 
            'payment_status' => 'paid', 
            'order_id' => sanitizeInput($_SESSION['payment_success_order_id'])
        ];
    } else if (isset($_SESSION['payment_failed_order_id'])) {
        $response = [
            'success' => true, 
            'payment_status' => 'failed', 
            'order_id' => sanitizeInput($_SESSION['payment_failed_order_id'])
        ];
    } else if (isset($_SESSION['current_temp_order_id'])) {
        $response = [
            'success' => true, 
            'payment_status' => 'pending_payment', 
            'temp_order_id' => sanitizeInput($_SESSION['current_temp_order_id'])
        ];
    }
...
if (isset($_GET['get_summary'])) {
    ...
    $order_id = validateInteger($_SESSION['payment_success_order_id'] ?? 0, 1);
    
    // If no successful payment, check current order ID
    if (!$order_id) {
        $order_id = validateInteger($_SESSION['current_order_id'] ?? 0, 1);
    }
        ...
        try {
            ...
            if ($order_info) {
                $data['order_info'] = [
                    ...
                    'email' => sanitizeInput($order_info['email']),
                    'phone' => sanitizeInput($order_info['phone']),
                    'payment_status' => sanitizeInput($order_info['payment_status']),
                    'total_amount' => (float)$order_info['total_amount'],
                    'currency' => sanitizeInput($order_info['currency']),
                    'visa_status' => sanitizeInput($order_info['visa_status']),
                    'created_at' => sanitizeInput($order_info['created_at'])
                ];
                $data['country_name'] = sanitizeInput($order_info['country_name']);
                ...
                if ($payment_info) {
                    $data['payment_info'] = [
                        'provider' => sanitizeInput($payment_info['provider']),
                        'provider_payment_id' => sanitizeInput($payment_info['provider_payment_id']),
                        'amount' => (float)$payment_info['amount'],
                        'currency' => sanitizeInput($payment_info['currency']),
                        'status' => sanitizeInput($payment_info['status']),
                        'created_at' => sanitizeInput($payment_info['created_at'])
                    ];
                }
                ...
                foreach ($applicants as $applicant) {
                    $applicant_key = "applicant_" . (int)$applicant['applicant_no'];
                    
                    $data[$applicant_key] = [
                        'email' => sanitizeInput($applicant['applicant_email']),
                        'phone' => sanitizeInput($applicant['applicant_phone']),
                        'visa_status' => sanitizeInput($applicant['visa_status']),
                        'answers' => []
                    ];
                    ...
                    foreach ($answers as $answer) {
                        $data[$applicant_key]['answers'][(int)$answer['question_id']] = sanitizeInput($answer['answer_text']);
                        if (!isset($data['question_labels'][$answer['question_id']])) {
                            $data['question_labels'][(int)$answer['question_id']] = sanitizeInput($answer['label']);
                        }
                    }
                    ...
                    foreach ($files as $file) {
                        $data[$applicant_key]['answers'][(int)$file['question_id']] = sanitizeInput($file['file_path']);
                        if (!isset($data['question_labels'][$file['question_id']])) {
                            $data['question_labels'][(int)$file['question_id']] = sanitizeInput($file['label']);
                        }
                    }
                }
            ...
            echo json_encode(['success' => false, 'message' => 'Error fetching summary']);
        }
    } else {
...
function fallbackToSessionData() {
    ...
    if (isset($_SESSION['collected_info']) && !empty($_SESSION['collected_info'])) {
        $sessionData = $_SESSION['collected_info'];
        
        // Sanitize session data
        $data = [];
        foreach ($sessionData as $key => $value) {
            if (is_array($value)) {
                $data[$key] = array_map('sanitizeInput', $value);
            } else {
        ...
        if (isset($_SESSION['question_data'])) {
            foreach ($_SESSION['question_data'] as $q_id => $q_data) {
                $question_labels[(int)$q_id] = sanitizeInput($q_data['label']);
            }
        }
        ...
        echo json_encode([
            ...
            'country_name' => sanitizeInput($_SESSION['country_name'] ?? ''),
            'order_info' => [
                'email' => sanitizeInput($_SESSION['order_contact_email'] ?? ''),
                'phone' => $order_contact_phone,
                'id' => sanitizeInput($_SESSION['payment_success_order_id'] ?? ($_SESSION['current_order_id'] ?? 'TMP')),
                'total_people' => (int)($_SESSION['total_people'] ?? 1),
                'payment_status' => isset($_SESSION['payment_success_order_id']) ? 'paid' : 'pending',
                'currency' => 'INR',
                'total_amount' => (float)($_SESSION['payment_amount'] ?? 0),
...
if (isset($_SESSION['payment_failed_temp_order_id'])) {
    $failed_temp_id = sanitizeInput($_SESSION['payment_failed_temp_order_id']);
...
if (isset($_SESSION['payment_success_order_id'])) {
    $success_order_id = sanitizeInput($_SESSION['payment_success_order_id']);
...
if (isset($_SESSION['payment_failed_order_id'])) {
    $failed_order_id = sanitizeInput($_SESSION['payment_failed_order_id']);
...
function formatBold($text) {
    ...
    return preg_replace('/\*\*(.*?)\*\*/', '<b>$1</b>', $safeText);
}
...
function applyValidationRules($value, $validation_rules, $question_label = '', $field_key = '', $context_data = [], $country_name = '') {
    ...
    if (isset($rules['min_length']) && strlen($value) < $rules['min_length']) {
        $errors[] = sprintf("Minimum %d characters required.", (int)$rules['min_length']);
    }
    
    if (isset($rules['max_length']) && strlen($value) > $rules['max_length']) {
        $errors[] = sprintf("Maximum %d characters allowed.", (int)$rules['max_length']);
    }
    ...
    if (isset($rules['regex']) && !empty($value)) {
        ...
        error_log("Validating field_key: $field_key with value: $value against pattern: $regex_pattern");
        
        // Remove delimiters if they exist
        if (substr($regex_pattern, 0, 1) === '/' && substr($regex_pattern, -1) === '/') {
            $regex_pattern = substr($regex_pattern, 1, -1);
        }
        
        // Escape forward slashes in the pattern for preg_match
        $regex_pattern = str_replace('/', '\/', $regex_pattern);
        
        // Test the pattern
        $test_result = @preg_match('/' . $regex_pattern . '/', $value);
        
        if ($test_result === false) {
            // Pattern is invalid
            error_log("Invalid regex pattern: " . $rules['regex']);
        } elseif ($test_result === 0) {
            // Pattern doesn't match
            if ($field_key === 'passport_number') {
                $errors[] = "Passport number must contain only uppercase letters and numbers (no spaces or special characters). Example: AB1234567";
                ...
                $errors[] = "Invalid format. Please check the format and try again.";
            }
        }
    }
    ...
    if (isset($rules['min_date'])) {
        ...
        if ($date_obj < $min_date) {
            if ($rules['min_date'] === 'TODAY') {
                ...
                return sprintf("Date must be on or after %s.", htmlspecialchars($min_date_str));
            }
        }
    }
    ...
    if (isset($rules['max_date'])) {
        ...
        if ($date_obj > $max_date) {
            if ($rules['max_date'] === 'TODAY') {
                ...
                return sprintf("Date must be on or before %s.", htmlspecialchars($max_date_str));
            }
        }
    }
...
function validateFileUpload($file, $validation_rules, $field_key = '') {
    ...
    if (!validateFileName($file['name'])) {
        $errors[] = "Invalid file type. Allowed formats: JPG, JPEG, PNG, PDF.";
        return $errors;
    }
    ...
    // Check for PDF malware
    if ($file_extension === 'pdf') {
        // Simple check: PDF should start with %PDF
...
function getSelectOptions($question_id, $pdo) {
    ...
    $db_options = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    foreach ($db_options as $option) {
        $options[] = [
            'option_value' => htmlspecialchars($option['option_value'], ENT_QUOTES, 'UTF-8'),
            'option_label' => htmlspecialchars($option['option_label'], ENT_QUOTES, 'UTF-8')
        ];
    }
    return $options;
}
    ...
    if (isset($collected_info["applicant_$p_num"]['answers'])) {
        foreach ($collected_info["applicant_$p_num"]['answers'] as $q_id => $answer) {
            if ($q_id != $current_q_id && isset($question_data[$q_id])) {
                $context[$question_data[$q_id]['field_key']] = htmlspecialchars($answer, ENT_QUOTES, 'UTF-8');
                $context[$question_data[$q_id]['label']] = htmlspecialchars($answer, ENT_QUOTES, 'UTF-8');
            }
        }
    }
...
if (isset($_GET['ajax'])) {
    // Verify this is a POST request for file uploads
    ...
    $msg = isset($_POST['message']) ? sanitizeInput(trim($_POST['message'])) : '';
    $response = "";
    $img_path = "";
    $progress = 0;
    ...
    if ($is_select_selection) {
        $msg = sanitizeInput($_POST['select_value']);
    }
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === 0) {
        ...
        if (!isset($_SESSION['order_folder_name'])) {
            $_SESSION['order_folder_name'] = 'TMP_' . time() . '_' . bin2hex(random_bytes(8));
        }

        $p_num = (int)($_SESSION['current_person_num'] ?? 1);
        $sub_path = date('Y/m/d') . '/' . $_SESSION['order_folder_name'] . '/applicant_' . $p_num;
        $full_dir = $base_gov_id . $sub_path . '/';
        
        // Create directory with secure permissions
        if (!is_dir($full_dir)) {
            if (!mkdir($full_dir, 0750, true)) {
        ...
        // Validate file name
        if (!validateFileName($original_name)) {
        ...
        if ($_SESSION['step'] === 'details' && isset($_SESSION['db_questions'][$_SESSION['q_idx']])) {
            ...
            $upload_errors[] = "File upload not allowed at this step.";
        }
        
        if (empty($upload_errors)) {
            // Additional security: move uploaded file
            if (move_uploaded_file($_FILES['image']['tmp_name'], $target)) {
                // Set secure permissions
                chmod($target, 0640);
                
                $relative_file_path = $sub_path . '/' . $filename;
                $img_path = '/fetch_file.php?path=' . urlencode($relative_file_path) . '&token=' . urlencode(bin2hex(random_bytes(16)));
            } else {
                $upload_errors[] = "Failed to save file.";
            }
        }
        ...
        switch ($_SESSION['step']) {
            case 'country':
                ...
                if ($country) {
                    ...
                    $_SESSION['country_name'] = sanitizeInput($country['country_name']);
                    ...
                    $q_stmt->execute([(int)$country['id']]);
                    $_SESSION['db_questions'] = $q_stmt->fetchAll();
                    
                    $_SESSION['question_data'] = [];
                    foreach ($_SESSION['db_questions'] as $q) {
                        $_SESSION['question_data'][(int)$q['id']] = [
                            'label' => sanitizeInput($q['label']),
                            'field_key' => sanitizeInput($q['field_key']),
                            'field_type' => sanitizeInput($q['field_type']),
                            'validation_rules' => $q['validation_rules']
                        ];
                        
                        if ($q['field_type'] === 'select') {
                            $_SESSION['question_data'][(int)$q['id']]['options'] = getSelectOptions($q['id'], $pdo);
                        }
                    }
                    ...
                    $response = "Sorry we currently don't support " . htmlspecialchars($msg) . ". Please try another Country."; 
                }
                break;

            case 'how_many':
                $numApplicants = validateInteger($msg, 1, 20);
                if ($numApplicants !== false) {
                    $_SESSION['total_people'] = $numApplicants;
                    $_SESSION['current_person_num'] = 1; 
                    $_SESSION['q_idx'] = 0; 
                    $_SESSION['step'] = 'details';
                    
                    $first_q = $_SESSION['db_questions'][0];
                    $first_q_id = (int)$first_q['id'];
                    $first_q_label = sanitizeInput($first_q['label']);
                    $first_field_type = sanitizeInput($first_q['field_type']);
            ...
            case 'details':
                ...
                $current_q_id = (int)$current_q['id'];
                $current_q_label = sanitizeInput($current_q['label']);
                $current_field_type = sanitizeInput($current_q['field_type']);
                $validation_rules = $current_q['validation_rules'] ?? '';
                $field_key = sanitizeInput($current_q['field_key'] ?? '');

                // DEBUG: Log what field we're validating
                error_log("DEBUG: Validating field_key: $field_key, field_type: $current_field_type, label: $current_q_label");
                ...
                if ($current_field_type === 'file' && !$img_path) {
                    ...
                    if (!empty($validation_errors)) {
                        ...
                        $_SESSION['collected_info']["applicant_$p_num"]['answers'][$current_q_id] = $img_path ?: sanitizeInput($msg);
                        $_SESSION['q_idx']++;

                        if ($_SESSION['q_idx'] < count($questions)) {
                            $next_q = $questions[$_SESSION['q_idx']];
                            $next_q_id = (int)$next_q['id'];
                            $next_q_label = sanitizeInput($next_q['label']);
                            $next_field_type = sanitizeInput($next_q['field_type']);
            ...
            case 'applicant_email':
                if (!validateEmail($msg)) {
                    ...
                    $_SESSION['collected_info']["applicant_$p_num"]['email'] = sanitizeInput($msg);
            ...
            case 'applicant_phone':
                ...
                if ($validation_error) {
                    ...
                    $_SESSION['collected_info']["applicant_$p_num"]['phone'] = sanitizeInput($msg);
            ...
            case 'order_email':
                if (!validateEmail($msg)) {
                    ...
                    $_SESSION['order_contact_email'] = sanitizeInput($msg);
            ...
            case 'order_phone':
                ...
                if ($validation_error) {
                    ...
                    $_SESSION['order_contact_phone'] = sanitizeInput($msg);
                    ...
                    $_SESSION['payment_amount'] = (float)$payment_info['amount'];
    ...
    switch ($_SESSION['step']) {
        ...
        case 'payment':
            ...
            $payment_amount = (float)($_SESSION['payment_amount'] ?? 0);
    ...
    echo json_encode([
        ...
        'current_person' => (int)($_SESSION['current_person_num'] ?? 1),
        'total_people' => (int)($_SESSION['total_people'] ?? 1),
        'show_date_calendar' => $show_date_calendar,
        'show_select_dropdown' => $show_select_dropdown,
        'select_options' => $select_options,
        'current_question_id' => isset($_SESSION['db_questions'][$_SESSION['q_idx']]['id']) ? (int)$_SESSION['db_questions'][$_SESSION['q_idx']]['id'] : null,
        'order_id' => sanitizeInput($_SESSION['payment_success_order_id'] ?? ($_SESSION['current_order_id'] ?? null)),
...
if (isset($_POST['reset'])) {
    ...
    if (!verifyCSRFToken($csrfToken)) {
        http_response_code(403);
        echo '<script>alert("CSRF token validation failed"); window.location.href="' . htmlspecialchars($_SERVER['PHP_SELF']) . '";</script>';
        exit;
    }
    ...
    <div class="chat-section">
        ...
        <div class="chat-container" id="chat">
            <?php foreach ($_SESSION['messages'] as $m): ?>
                <div class="message-row <?php echo htmlspecialchars($m['role']); ?>">
                    <?php if ($m['role'] === 'bot'): ?>
                        ...
                        <?php if (isset($m['img']) && $m['img']): ?>
                            <div class="message-attachment">
                                <?php if (isset($m['is_pdf']) && $m['is_pdf']): ?>
                                    <div class="pdf-card" onclick="openLightbox('<?php echo htmlspecialchars($m['img']); ?>', true)">
                                ...
                                <?php else: ?>
                                    <img src="<?php echo htmlspecialchars($m['img']); ?>" class="msg-img" onclick="openLightbox(this.src)">
                                <?php endif; ?>
                            </div>
                        <?php endif; ?>
...
<script>
    ...
    function createSelectDropdown(questionId, options) {
        ...
        options.forEach(option => {
            ...
            optionsHtml += `
                <div class="select-option" onclick="selectOption('${questionId}', '${value.replace(/'/g, "\\'")}', '${label.replace(/'/g, "\\'")}')">
                    <span>${label}</span>
        ...
        .then(data => {
            ...
            if (data.step === 'payment') {
                ...
                if (botMessages.length > 0) {
                    ...
                    if (messageContent && !messageContent.querySelector('.payment-container')) {
                        // Get payment amount from session (PHP will output it)
                        const paymentAmount = <?php echo $_SESSION['payment_amount'] ?? 0; ?>;
                        const currency = '<?php echo $_SESSION['currency'] ?? 'INR'; ?>';
                        
                        if (paymentAmount > 0) {
                            const paymentButton = createPaymentButton(paymentAmount, currency);
                            messageContent.appendChild(paymentButton);
