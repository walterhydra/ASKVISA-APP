    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        html.dark {
            ...
            --text-main: #f1f5f9;
            --text-muted: #94a3b8;
            --bg-light: rgba(30, 41, 59, 0.7);
            --bg-alt: rgba(15, 23, 42, 0.5);
...
<script>
        ...
        // When the reset form is submitted, set flag and clear storage
        document.addEventListener('DOMContentLoaded', () => {
            const resetBtn = document.querySelector('button[name="reset"]');
            if (resetBtn) {
                resetBtn.addEventListener('click', () => {
                    isResetting = true;
                    sessionStorage.removeItem('askvisa_chat');
                    sessionStorage.removeItem('askvisa_progress');
                });
            }
        });
        ...
        async function showSummary() {
            try {
                ...
                if (data.success) {
                        ...
                        `;
                            ...
                            if (applicant.answers) {
                                Object.keys(applicant.answers).forEach(qId => {
                                    ...
                                    const answerText = answer === null || answer === undefined ? '' : String(answer);
                                    ...
                                    if (answerText.includes('fetch_file.php')) {
                                        const isPdf = answerText.includes('.pdf') || answerText.toLowerCase().includes('pdf');
                                        const fileName = getFileNameFromPath(answerText);

                                        // Ensure the URL is properly formatted
                                        let fileUrl = answerText;
                                        ...
                                        <div class="summary-file">
                                            ...
                                            <div class="file-preview">
                                                ${!isPdf ? `
                                                    ...
                                                    <p style="font-size: 11px; color: var(--text-muted); margin-top: 4px;">Click to preview</p>
                                                ` : `
                                                    <div onclick="openLightbox('${fileUrl}', true)" style="cursor: pointer; text-align: center;">
                                                        ...
                                                        <p style="font-size: 12px; color: var(--text-main); font-weight: 600;">PDF Document</p>
                                                        <p style="font-size: 10px; color: var(--text-muted);">Click to view</p>
                                                    </div>
                                                `}
                                            </div>
                                        </div>
                                    `;
                                        ...
                                        <div class="summary-item">
                                            ...
                                            <span class="summary-value">${answerText || 'N/A'}</span>
                                        </div>
                                    `;
                                    }
                                });
                    ...
                    const paymentStatus = (data.order_info && data.order_info.payment_status)
                        ? String(data.order_info.payment_status).toLowerCase()
                        : '';
                    if (paymentStatus === 'paid' || paymentStatus === 'success' || paymentStatus === 'successful' || paymentStatus === 'captured') {
        ...
        `;
            ...
            try {
                ...
                window.open('generate_pdf.php?order_id=' + encodeURIComponent(orderId), '_blank');
        ...
        // ─── SessionStorage: Save & Restore Chat State ───
        function saveChatState() {
            try {
                sessionStorage.setItem('askvisa_chat', chat.innerHTML);
                sessionStorage.setItem('askvisa_progress', JSON.stringify({
                    width: pBar ? pBar.style.width : '0%',
                    percent: progressPercent ? progressPercent.textContent : '0%',
                    label: stepLabel ? stepLabel.textContent : '',
                    count: stepCount ? stepCount.textContent : '',
                    applicant: applicantCount ? applicantCount.textContent : '',
        ...
        function restoreChatState() {
            const savedChat = sessionStorage.getItem('askvisa_chat');
            const savedProgress = sessionStorage.getItem('askvisa_progress');
            // Only restore chat HTML if the server didn't already render messages
            if (savedChat && chat.querySelectorAll('.message-row').length <= 1) {
                chat.innerHTML = savedChat;
                // Re-append typing indicator (it was part of innerHTML but hidden)
            ...
            // Always restore progress/sidebar state
            if (savedProgress) {
                try {
                    const p = JSON.parse(savedProgress);
                    if (pBar) pBar.style.width = p.width;
                    if (progressPercent) progressPercent.textContent = p.percent;
                    if (stepLabel && p.label) stepLabel.textContent = p.label;
        ...
        // Save chat state before leaving the page (refresh, navigation, etc.)
        // sessionStorage persists across page refreshes and same-tab navigations,
        ...
        // Restore chat state on load (for returning from payment or page refresh)
        restoreChatState();
        ...
        function toggleDarkMode() {
            ...
            localStorage.setItem('askvisa_theme', isDark ? 'dark' : 'light');
        }

        // Restore theme on page load
        (function restoreTheme() {
            if (localStorage.getItem('askvisa_theme') === 'dark') {
