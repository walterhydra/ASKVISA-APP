...
if (!class_exists('InvoicePDF')) {
    class InvoicePDF extends TCPDF {
        ...
        public function Header() {
            ...
            if (file_exists($image_file) && (extension_loaded('gd') || extension_loaded('imagick'))) {
                try {
                    $this->Image($image_file, 15, 10, 30, '', 'PNG', '', 'T', false, 300, '', false, false, 0, false, false, false);
                } catch (Exception $e) {
                    // Ignore image error if it fails
                }
            }
...
function getInvoicePDFString($order_id, $pdo) {
    // Get order details
    $stmt = $pdo->prepare("
    ...
    $html = '<table border="0" cellpadding="5" cellspacing="0" style="width:100%;">
        ...
        <tr>
            ...
            <td style="border-bottom:1px solid #ddd;">' . htmlspecialchars($order['country_name'] ?? 'N/A') . '</td>
        </tr>
        <tr>
            <td style="border-bottom:1px solid #ddd;"><strong>Visa Type:</strong></td>
            <td style="border-bottom:1px solid #ddd;">' . htmlspecialchars($order['visa_type'] ?? 'Standard Visa') . '</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td style="border-bottom:1px solid #ddd;">' . htmlspecialchars($order['processing_time'] ?? 'Standard Processing') . '</td>
        </tr>
    </table>';
    ...
    $html = '<table border="0" cellpadding="5" cellspacing="0" style="width:100%;">
        <tr>
            ...
            <td width="60%" style="border-bottom:1px solid #ddd;">' . ($order['currency'] ?? 'INR') . ' ' . number_format($order['total_amount'] ?? 0, 2) . '</td>
        </tr>
        <tr>
            <td style="border-bottom:1px solid #ddd;"><strong>Payment Method:</strong></td>
            <td style="border-bottom:1px solid #ddd;">' . ($payment ? htmlspecialchars($payment['payment_method'] ?? 'Online Payment') : 'Online Payment') . '</td>
        </tr>
        <tr>
    ...
    if ($payment && isset($payment['provider_payment_id'])) {
        $html .= '<tr>
    ...
    $pdf->Cell(0, 15, 'Total Paid: ' . ($order['currency'] ?? 'INR') . ' ' . number_format($order['total_amount'] ?? 0, 2), 1, 1, 'C', true);
    $pdf->Ln(15);
...
6. This is a computer-generated invoice and does not require a signature.', 0, 'L');
    ...
    return $pdf->Output('invoice-order-#' . $order_id . '.pdf', 'S');
}
