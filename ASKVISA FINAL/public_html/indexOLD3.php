...
// LOGIC: Clear session if this is a new tab (triggered by JS below)
if (isset($_GET['new_tab'])) {
    ...
    // Redirect back to clean URL but with acknowledgment
    $params = $_GET;
    unset($params['new_tab']);
    $params['session_reset'] = 1;
    $query = http_build_query($params);
    header("Location: index.php" . ($query ? '?' . $query : ''));
    exit;
}

// LOGIC: If order is complete, refreshing index.php should start a NEW application (Reset Data).
// However, we must allow the user to see the "Finish" screen AT LEAST ONCE after redirection.
// So we check if they have ALREADY SEEN the finish screen (flag set below).
// BUT we must NOT clear the session if this is an AJAX request (like downloading summary).
if (isset($_SESSION['payment_success_order_id']) && isset($_SESSION['order_complete_shown']) && !isset($_GET['get_summary']) && !isset($_REQUEST['ajax'])) {
    session_unset();
    session_destroy();
    session_start();
    $csrf_token = generate_csrf_token();
...
if (isset($_GET['country']) && !empty($_GET['country'])) {
    ...
    // Check if we need to switch country or start fresh
    // If different country OR just want to restart flow for same country

    // Try to find this country in the database
    $stmt = $pdo->prepare("SELECT id, country_name, country_code FROM countries WHERE country_name LIKE ?");
    ...
    if ($country) {
        ...
        if (!$is_resuming) {
            ...
            if (empty($visa_types)) {
                ...
                // The frontend expects: json_select:question_id:Message Text
                // And a separate 'select_options' array in the AJAX response

                // We'll store options in session to be sent via AJAX
    ...
    // If country not found, standard flow will catch below if session not set
    else {
        ...
        // Force the frontend to show the country selection (step=country) which effectively asks them to pick again
    }
}
...
function ensureVisaTypesTableExists($pdo)
{
    ...
    if ($thailand) {
        ...
        if ($stmt->fetchColumn() == 0) {
            $sql = "INSERT INTO visa_types (country_id, name, price, currency, description) VALUES
                (?, 'Tourist Visa (30 Days)', 2500.00, 'INR', 'Process Time: 3-4 days'),
                (?, 'Visa on Arrival', 2000.00, 'INR', 'Pay at airport'),
                (?, 'Business Visa', 5000.00, 'INR', 'Valid for 90 days')";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$thailand_id, $thailand_id, $thailand_id]);
        }
    }
}
...
if (isset($_GET['ajax'])) {
    ...
    if ($msg !== '' || $img_path !== '' || $is_select_selection) {
        ...
        switch ($_SESSION['step']) {
            ...
            case 'visa_type':
                ...
                // Method 1: ID from Dropdown (json_select)
                ...
                if (!$found_visa) {
                    ...
                    if (is_numeric($selection) && intval($selection) > 0 && intval($selection) <= count($visa_types)) {
                        ...
                        // Check if name selection
                        foreach ($visa_types as $vt) {
                            if (stripos($vt['name'], $selection) !== false) {
                                $found_visa = $vt;
                                break;
                            }
                        }
                ...
                if ($found_visa) {
                    ...
                    $stmt = $pdo->prepare("SELECT id, label, field_type, validation_rules, field_key FROM country_questions WHERE country_id = ? ORDER BY sort_order ASC");
    ...
    switch ($_SESSION['step']) {
        ...
        case 'visa_type':
            ...
            $country_name = $_SESSION['country_name'] ?? 'Thailand';
            // Get Country ID
...
<head>
    ...
    <script>
        (function () {
            // LOGIC: Check if this is a "new" tab session (survives refresh, dies on close)
            // If sessionStorage is empty, it means the tab was closed or it's a new tab.
            if (!sessionStorage.getItem('visa_app_session_active')) {
                const urlParams = new URLSearchParams(window.location.search);

                // If we just came back from a server-side reset (indicated by session_reset param)
                if (urlParams.has('session_reset')) {
                    console.log('Session reset confirmed. Marking tab as active.');
                    sessionStorage.setItem('visa_app_session_active', 'true');

                    // Clean the URL (remove session_reset param)
                    const newUrl = window.location.pathname + window.location.search.replace(/[?&]session_reset=1/, '').replace(/^&/, '?');
                    window.history.replaceState({}, document.title, newUrl);
                } else {
                    // No session flag and no reset confirmation -> This is a new/reopened tab.
                    // We must tell the server to clear the PHP session.
                    console.log('New tab detected. Resetting session...');
                    urlParams.append('new_tab', '1');
                    window.location.search = urlParams.toString();
                }
            } else {
                console.log('Active tab session detected. Retaining data.');
            }
        })();
    </script>
    ...
    <style>
        ...
        * {
        ...
        .sidebar {
            ...
            z-index: 100;
            /* Increased z-index for mobile overlay */
            position: relative;
            overflow-y: auto;
...
/* Removed spinning animation as per request */
        /*
        ...
        */
        ...
        .summary-container {
            ...
            max-height: 90vh;
            display: flex;
            /* Enable flexbox for internal layout */
            flex-direction: column;
            /* Stack top-to-bottom */
            box-shadow: var(--box-shadow-lg);
            border: 1px solid var(--gray-light);
        ...
        @media (max-width: 767px) {
            ...
            /* Hide Sidebar Elements in Compact Mode */
            ...
            /* Add a class to expand sidebar if we want to show menu later, 
               but for now user wants simple responsive without horizontal scroll */
            ...
            .input-field {
                font-size: 15px;
                /* Prepare for iOS zoom prevention */
                padding: 10px 0;
            }
        ...
        /* Select Dropdown Styles (Visa Type Cards) */
        .select-container {
            margin-top: 15px;
            display: flex;
            flex-direction: column;
        ...
        /* Dark mode adjustments for select options */
        ...
        <div class="chat-section">
            ...
            <div class="input-section">
                <div class="input-wrapper">
                    ...
                    <input type="text" id="msgInput" class="input-field" placeholder="Type your response here..."
                        autocomplete="off">
