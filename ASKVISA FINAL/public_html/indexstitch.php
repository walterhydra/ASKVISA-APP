...
<head>
            ...
            theme: {
                extend: {
                    colors: {
                        ...
                        "background-dark": "#09090b", // Deep Grayish Black
                        "glass-light": "rgba(255, 255, 255, 0.7)",
                        "glass-dark": "rgba(24, 24, 27, 0.7)", // Zinc-900 transparent
                    },
                    fontFamily: {
                        "display": ["Inter", "sans-serif"]
                    },
                    borderRadius: {
                        "DEFAULT": "1rem", "lg": "1.5rem", "xl": "2rem", "2xl": "2.5rem", "full": "9999px"
                    },
                },
            },
        }
    ...
    <style>
        ...
        ::-webkit-scrollbar-thumb { background: rgba(156, 163, 175, 0.3); border-radius: 999px; }
        ...
        .loading-dot { width: 6px; height: 6px; border-radius: 50%; background: #9ca3af; animation: dotPulse 1.4s infinite ease-in-out; }
        .loading-dot:nth-child(1) { animation-delay: -0.32s; }
        .loading-dot:nth-child(2) { animation-delay: -0.16s; }
        @keyframes dotPulse { 0%, 80%, 100% { transform: scale(0); } 40% { transform: scale(1); } }
        
        /* Visa Cards Override for output */
        .select-container { display: flex; flex-direction: column; gap: 8px; width: 100%; max-width: 400px; margin-top: 10px; }
        .select-option { 
            background: rgba(255, 255, 255, 0.8); border: 1px solid rgba(220, 38, 38, 0.2); 
        ...
        html.dark .select-option { background: rgba(39, 39, 42, 0.8); border-color: rgba(255,255,255,0.1); color: #f3f4f6; }
        .select-option:hover { background: #DC2626; color: white; border-color: #DC2626; transform: translateY(-2px); }
        .select-option.selected { background: #DC2626; color: white; border-color: #DC2626; }
        ...
        .lightbox-close { position: absolute; top: 20px; right: 30px; color: white; font-size: 30px; cursor: pointer; }
    </style>
</head>
<body id="body" class="font-display text-slate-900 dark:text-slate-100 min-h-screen flex overflow-hidden">
    
    <!-- Sidebar -->
    <aside class="w-80 hidden md:flex flex-col border-r border-slate-200 dark:border-white/10 bg-white/60 dark:bg-glass-dark glass m-4 rounded-3xl z-10 shadow-xl overflow-hidden">
        <div class="p-6 border-b border-slate-200 dark:border-white/10 flex items-center gap-3">
            ...
            <div>
                <h1 class="text-lg font-bold leading-tight">Visa Concierge</h1>
                <div class="flex items-center gap-1.5">
                    <span class="size-2 rounded-full bg-green-500 shadow-[0_0_8px_rgba(16,185,129,0.8)]"></span>
                    <span class="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Premium Support</span>
                </div>
            </div>
        </div>

        <div class="p-6 border-b border-slate-200 dark:border-white/10 space-y-4">
            <div class="flex justify-between items-end">
                <div>
                    <p class="text-[10px] font-bold text-primary uppercase tracking-widest" id="stepCount">Step 1/9</p>
                    <p class="text-sm font-semibold text-slate-800 dark:text-slate-200" id="stepLabel">Country Selection</p>
                </div>
                <p class="text-[10px] font-semibold text-slate-500" id="progressPercent">0%</p>
            </div>
            <div class="w-full bg-slate-200 dark:bg-black/40 h-2 rounded-full overflow-hidden shadow-inner">
                <div id="pBar" class="bg-primary h-full rounded-full transition-all duration-500" style="width: 0%;"></div>
            </div>
            <p class="text-[10px] font-medium text-slate-500 text-center" id="applicantCount">Applicants: 0</p>
        </div>

        <div class="p-6 flex-1 flex flex-col gap-3">
            <button onclick="toggleConfirm(true)" class="w-full bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:border-primary/50 text-sm font-semibold py-3 px-4 rounded-xl flex items-center justify-center gap-2 transition-all">
                <i class="fas fa-plus-circle text-primary"></i> New Application
            </button>
            <button onclick="window.location.href='edit.php'" class="w-full bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:border-primary/50 text-sm font-semibold py-3 px-4 rounded-xl flex items-center justify-center gap-2 transition-all">
                <i class="fas fa-edit text-primary"></i> Edit Existing Order
            </button>
            <button onclick="showSummary()" class="w-full bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:border-primary/50 text-sm font-semibold py-3 px-4 rounded-xl flex items-center justify-center gap-2 transition-all">
                <i class="fas fa-download text-primary"></i> Download Summary
            </button>
            <button onclick="window.location.href='privacy_policy.php'" class="w-full bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:border-primary/50 text-sm font-semibold py-3 px-4 rounded-xl flex items-center justify-center gap-2 transition-all">
                <i class="fas fa-shield-alt text-primary"></i> Privacy Policy
            </button>
        </div>

        <div class="p-6 mt-auto">
            <div class="flex items-center justify-between bg-white/50 dark:bg-black/20 p-3 rounded-2xl border border-slate-200 dark:border-white/5">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" id="themeToggle" class="sr-only peer" onchange="toggleTheme(this.checked)">
    ...
    <!-- Main Chat Section -->
    <main class="flex-1 flex flex-col h-screen md:py-4 md:pr-4 relative z-0">
        <!-- Mobile Header (Visible only on small screens) -->
        <header class="md:hidden sticky top-0 z-50 bg-white/80 dark:bg-background-dark/80 glass border-b border-slate-200 dark:border-white/10 px-4 py-3 flex items-center justify-between">
            ...
            <button class="size-10 rounded-full bg-white dark:bg-primary/20 border border-slate-200 dark:border-primary/30 flex items-center justify-center transition-colors" onclick="toggleTheme()">
                <span class="material-symbols-outlined">dark_mode</span>
            </button>
        </header>

        <!-- Chat Area -->
        <div class="flex-1 overflow-y-auto px-4 py-6 space-y-6 scroll-smooth" id="chat">
            
            <?php foreach ($_SESSION['messages'] as $m): ?>
                <?php if ($m['role'] === 'bot'): ?>
                    <!-- Bot Message -->
                    <div class="flex items-start gap-3 w-full">
                        <div class="size-9 rounded-full bg-slate-200 dark:bg-slate-800 flex items-center justify-center shrink-0 border border-slate-300 dark:border-slate-700 shadow-sm mt-1">
                            <span class="material-symbols-outlined text-sm text-slate-700 dark:text-slate-300">robot_2</span>
                        </div>
                        <div class="flex flex-col gap-1.5 max-w-[85%]">
                            <div class="bg-white/80 dark:bg-glass-dark glass border border-slate-200 dark:border-white/10 p-4 rounded-3xl rounded-tl-none shadow-sm">
                                ...
                                <?php if (isset($m['img']) && $m['img']): ?>
                                    <div class="mt-3">
                                        <?php if (isset($m['is_pdf']) && $m['is_pdf']): 
                                            ...
                                            ?>
                                            <div class="bg-slate-100 dark:bg-black/40 border border-slate-200 dark:border-white/5 p-3 rounded-2xl flex items-center gap-3 cursor-pointer hover:bg-slate-200 dark:hover:bg-black/60 transition-colors" onclick="openLightbox('<?php echo $preview_url; ?>', true)">
                                                <i class="fas fa-file-pdf text-primary text-2xl"></i>
                                                <div>
                                                    <h4 class="text-sm font-semibold truncate max-w-[150px]"><?php echo $pdf_name; ?></h4>
                                                    <p class="text-[10px] text-slate-500">Tap to view</p>
                                                </div>
                                            </div>
                                        <?php else: ?>
                                            <img src="<?php echo $m['img']; ?>" class="max-w-[200px] rounded-xl cursor-pointer border border-slate-200 dark:border-white/10 shadow-sm" onclick="openLightbox(this.src)">
                                        <?php endif; ?>
                                    </div>
                                <?php endif; ?>
                            </div>
                            <span class="text-[10px] font-medium text-slate-500 dark:text-slate-400 px-2"><?php echo date('H:i'); ?> • Bot</span>
                        </div>
                    </div>
                <?php else: ?>
                    <!-- User Message -->
                    <div class="flex flex-col items-end gap-1.5 w-full">
                        <div class="flex items-start gap-3 flex-row-reverse w-full">
                            <div class="size-9 rounded-full bg-primary flex items-center justify-center shrink-0 border border-primary/20 shadow-sm mt-1">
                                <span class="material-symbols-outlined text-white text-sm">person</span>
                            </div>
                            <div class="flex flex-col gap-1.5 max-w-[85%] items-end">
                                <div class="bg-primary text-white p-4 rounded-3xl rounded-tr-none shadow-lg shadow-primary/30">
                                    <div class="text-sm leading-relaxed font-medium">
                                        <?php echo formatBold($m['text']); ?>
                                    </div>
                                    <?php if (isset($m['img']) && $m['img']): ?>
                                        <div class="mt-3">
                                            <img src="<?php echo $m['img']; ?>" class="max-w-[200px] rounded-xl cursor-pointer border border-white/20 shadow-sm" onclick="openLightbox(this.src)">
                                        </div>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                        <span class="text-[10px] font-medium text-slate-500 dark:text-slate-400 px-2"><?php echo date('H:i'); ?> • You</span>
                    </div>
                <?php endif; ?>
            <?php endforeach; ?>

            <div id="completionState" style="display: none;" class="w-full flex justify-center py-6">
                <div class="bg-white/80 dark:bg-black/40 glass p-8 rounded-3xl border border-success/30 border-l-4 border-l-success shadow-lg text-center max-w-sm w-full mx-auto">
                    <div class="size-16 rounded-full bg-success/10 flex items-center justify-center text-success mx-auto mb-4 text-3xl">
                        <i class="fas fa-check-circle"></i>
                    </div>
                    <h3 class="text-xl font-bold mb-2">Application Complete!</h3>
                    <p class="text-sm text-slate-500 dark:text-slate-400 mb-4">Your visa application has been successfully submitted.</p>
                    <div class="text-2xl font-mono font-bold text-primary bg-primary/10 py-2 px-4 rounded-xl inline-block" id="finalOrderId">#0000</div>
                    <p class="text-xs text-slate-400 mt-4">You will receive a confirmation email shortly.</p>
                </div>
            </div>

            <!-- Typing Indicator (injected by JS) -->
            <div id="typingIndicator" style="display: none;" class="flex items-start gap-3 w-full opacity-50">
                <div class="size-8 rounded-full bg-slate-200 dark:bg-slate-800 flex items-center justify-center shrink-0 border border-slate-300 dark:border-slate-700">
                    <span class="material-symbols-outlined text-sm">robot_2</span>
                </div>
                <div class="bg-white/80 dark:bg-glass-dark glass border border-slate-200 dark:border-white/10 p-3 rounded-2xl rounded-tl-none flex items-center gap-2">
        ...
        <!-- Preview Tray -->
        <div id="previewTray" class="hidden absolute bottom-24 left-4 right-4 md:left-auto md:right-8 md:w-80 bg-white dark:bg-zinc-800 rounded-2xl p-3 border border-slate-200 dark:border-white/10 shadow-2xl glass flex items-center gap-3 z-40 transform translate-y-2 opacity-0 transition-all duration-300">
            <img id="previewImg" src="" class="size-12 object-cover rounded-xl border border-slate-200 dark:border-zinc-700">
            <div class="flex-1 overflow-hidden">
                <h4 id="previewFileName" class="text-sm font-bold truncate">File Preview</h4>
                <p id="previewFileSize" class="text-[10px] text-slate-500">Ready to upload</p>
            </div>
            <button onclick="clearPreview()" class="size-8 rounded-full bg-slate-100 dark:bg-zinc-700 hover:bg-slate-200 dark:hover:bg-zinc-600 flex items-center justify-center text-slate-500 transition-colors">
                <i class="fas fa-times"></i>
            </button>
        </div>

        <!-- Input Area -->
        <footer class="p-4 bg-white/50 dark:bg-background-dark/80 backdrop-blur-xl border-t border-slate-200 dark:border-white/10 md:rounded-b-3xl">
            <div class="max-w-4xl mx-auto flex items-end gap-3">
                <div class="relative flex-1 bg-white/80 dark:bg-black/40 border border-slate-200 dark:border-white/10 rounded-[2rem] p-1.5 flex items-center shadow-inner glass focus-within:border-primary/50 focus-within:ring-1 focus-within:ring-primary/50 transition-all">
                    <label id="attachBtn" class="size-11 rounded-full flex items-center justify-center text-slate-400 hover:text-primary hover:bg-primary/5 cursor-pointer transition-colors shrink-0">
                        <span class="material-symbols-outlined text-xl">attach_file</span>
                        <input type="file" id="fileInput" hidden accept="image/*,application/pdf" disabled onchange="handlePreview(this)">
                    </label>
                    <input type="text" id="msgInput" class="flex-1 bg-transparent border-none focus:ring-0 text-slate-800 dark:text-slate-100 placeholder:text-slate-400 text-sm py-3 px-2 focus:outline-none" placeholder="Type your message..." autocomplete="off">
                    <button class="md:hidden size-11 rounded-full flex items-center justify-center text-slate-400 hover:text-primary hover:bg-primary/5 transition-colors shrink-0">
                        <span class="material-symbols-outlined text-xl">mic</span>
                    </button>
                </div>
                <button id="sendBtn" onclick="sendMessage()" class="size-[60px] bg-primary text-white rounded-full flex items-center justify-center shadow-lg shadow-primary/30 hover:opacity-90 active:scale-95 transition-transform shrink-0">
                    <span class="material-symbols-outlined pl-1">send</span>
                </button>
            </div>
        </footer>
    ...
    <!-- Confirm Overlay -->
    <div id="confirmOverlay" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex justify-center items-center opacity-0 transition-opacity">
        <div class="bg-white dark:bg-zinc-900 w-full max-w-sm p-6 rounded-3xl shadow-2xl border border-slate-200 dark:border-white/5 transform scale-95 transition-transform text-center mx-4">
            <div class="size-16 rounded-full bg-danger/10 text-danger flex items-center justify-center mx-auto mb-4 text-2xl">
                <i class="fas fa-exclamation-triangle"></i>
            </div>
            <h3 class="text-lg font-bold mb-2">Reset Application?</h3>
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">This will clear all current progress and start a new application. This action cannot be undone.</p>
            <div class="flex flex-col gap-3">
                <form method="POST" class="w-full m-0">
                    <button type="submit" name="reset" class="w-full bg-danger text-white font-semibold py-3 px-4 rounded-xl shadow-lg shadow-danger/30 hover:opacity-90 active:scale-95 transition-all">Yes, Reset Now</button>
                </form>
                <button onclick="toggleConfirm(false)" class="w-full bg-slate-100 dark:bg-white/5 text-slate-700 dark:text-slate-200 font-semibold py-3 px-4 rounded-xl hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">Cancel</button>
            </div>
        </div>
    </div>

    <!-- Summary Popup -->
    <div id="summaryPopup" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex justify-center items-center opacity-0 transition-opacity">
        <div class="bg-white dark:bg-zinc-900 w-full max-w-2xl max-h-[85vh] rounded-3xl shadow-2xl border border-slate-200 dark:border-white/5 flex flex-col mx-4 overflow-hidden">
            <div class="p-6 border-b border-slate-200 dark:border-white/10 flex justify-between items-center bg-slate-50 dark:bg-white/5">
                <h3 class="text-xl font-bold">Application Summary</h3>
                <button onclick="closeSummaryPopup()" class="size-8 rounded-full bg-slate-200 dark:bg-white/10 hover:bg-slate-300 dark:hover:bg-white/20 flex items-center justify-center transition-colors">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <div class="p-6 overflow-y-auto flex-1 text-sm prose prose-sm dark:prose-invert max-w-none" id="summaryContent">
                <!-- Dynamic Content inserted here -->
            </div>
            <div class="p-6 border-t border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div class="flex items-center gap-3 w-full sm:w-auto">
                    <button id="downloadPdfBtn" onclick="downloadSummaryAsPDF()" class="flex-1 sm:flex-none bg-primary text-white font-semibold py-2.5 px-6 rounded-xl shadow-lg shadow-primary/30 hover:opacity-90 active:scale-95 transition-all flex justify-center items-center gap-2">
                        <i class="fas fa-download"></i> Download PDF
                    </button>
                    <button onclick="closeSummaryPopup()" class="flex-1 sm:flex-none bg-slate-200 dark:bg-white/10 font-semibold py-2.5 px-6 rounded-xl hover:bg-slate-300 dark:hover:bg-white/20 transition-all text-center">
                        Close
                    </button>
                </div>
...
<script>
            ...
            try {
                ...
                `;
                    ...
                    if (data.show_skip_button) {
                        ...
                        skipBtnContainer.className = 'skip-btn-container';
                        skipBtnContainer.innerHTML = `<button class="skip-btn" onclick="sendSkip()">Skip this optional step <i class="fas fa-forward"></i></button>`;
                        botRow.querySelector('.message-content').appendChild(skipBtnContainer);
                    }
        ...
        function createSelectDropdown(questionId, options) {
            ...
            options.forEach(opt => {
                ...
                if (priceMatch) {
                    optionDiv.innerHTML = `
                    <div class="option-name">${priceMatch[1]}</div>
                    <div class="option-price">${priceMatch[2]} ${priceMatch[3]}</div>
                `;
                } else {
                    optionDiv.textContent = labelText;
                }
