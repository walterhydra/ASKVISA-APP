...
try {
    ...
    $order_details = [
        ...
        ['Visa Type:', $order_info['visa_type'] ?? 'Standard Visa'],
        ['Processing Time:', $order_info['processing_time'] ?? 'Standard Processing'],
        ['Visa Status:', $order_info['visa_status'] ?? 'Pending'],
                ...
                foreach ($files[$applicant_no] as $qid => $file_info) {
                    ...
                    if ($file_type == 'image') {
                        ...
                        if (file_exists($full_path)) {
                            // Check if GD or Imagick is loaded
                            if (extension_loaded('gd') || extension_loaded('imagick')) {
                                ...
                                $pdf->Cell(0, 6, '[Image available but cannot be displayed in PDF (GD/Imagick missing): ' . $filename . ']', 0, 1, 'L');
                                $pdf->SetTextColor(0, 0, 0);
                                $pdf->Ln(5);
                            }
                        } else {
