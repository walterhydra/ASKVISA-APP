                                        ...
                                        if (isset($parsed_url['query'])) {
                                            ...
                                            if (isset($query_params['path'])) {
                                                ...
                                                if (file_exists($old_full_path)) {
                                                    ...
                                                    // Align naming with index.php (use only first name if possible)
                                                    $clean_name = !empty($first_name) ? preg_replace('/[^a-zA-Z0-9]/', '_', $first_name) : 'applicant';
                                                    $new_filename = $clean_name . '_' . $doc_type . '.' . $ext;
                                                    
                                                    // New directory structure: gov_id/YEAR/MONTH/DAY/Order_ID/applicant_no/
                                                    $date_path = date('Y/m/d');
                                                    $new_dir_relative = $date_path . '/Order_' . $real_order_id . '/applicant_' . $applicant_no;
                                                    $new_dir_full = $base_gov_id . $new_dir_relative;
                                                    
                                                    // HOSTINGER FIX: Ensure reliable directory creation and permissions
                                                    if (!is_dir($new_dir_full)) {
                                                        if (!mkdir($new_dir_full, 0755, true)) {
                                                            error_log("CRITICAL ERROR: Failed to create directory at $new_dir_full");
                                                        } else {
                                                            // Sometimes mkdir permission setting fails, forcing it via chmod
                                                            chmod($new_dir_full, 0755);
                                                        }
                                                    }
                                                    ...
                                                    if (file_exists($new_full_path)) {
                                                        $new_filename = $clean_name . '_' . $applicant_no . '_' . $doc_type . '.' . $ext;
                                                        $new_full_path = $new_dir_full . '/' . $new_filename;
                                                    }
                                                    
                                                    // HOSTINGER FIX: Add error logging to rename to see if it's failing silently
                                                    if (rename($old_full_path, $new_full_path)) {
                                                        $new_relative_path = $new_dir_relative . '/' . $new_filename;
                                                        $new_img_path = '/fetch_file.php?path=' . urlencode($new_relative_path);
                                                        
                                                        // Update session messages so chatbot images don't break
                                                        if (isset($_SESSION['messages'])) {
                                                            foreach ($_SESSION['messages'] as &$msg_item) {
                                                                if (isset($msg_item['img']) && $msg_item['img'] === $answer) {
                                                                    $msg_item['img'] = $new_img_path;
                                                                }
                                                            }
                                                        }
                                                        
                                                        // Update the answer that will be saved to DB
                                                        $answer = $new_img_path;
                                                        
                                                        // CRITICAL FIX: Update the session data so `index.php`'s get_summary and chatbot don't use old paths
                                                        if (isset($_SESSION['collected_info'][$applicant_key]['answers'][$question_id])) {
                                                            $_SESSION['collected_info'][$applicant_key]['answers'][$question_id] = $new_img_path;
                                                        }
                                                        if (isset($_SESSION['temp_application_data']['collected_info'][$applicant_key]['answers'][$question_id])) {
                                                            $_SESSION['temp_application_data']['collected_info'][$applicant_key]['answers'][$question_id] = $new_img_path;
                                                        }
                                                    } else {
                                                        error_log("CRITICAL ERROR: Failed to rename/move file from $old_full_path to $new_full_path. Check permissions.");
                                                    }
                                                } else {
                                                    error_log("CRITICAL ERROR: Source temporary file does not exist at $old_full_path. Answer path was: $answer");
                                                }
                                            }
                                        }
